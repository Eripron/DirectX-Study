...
#include "../Common/AnimationHelper.h"
...
public:
	...
	// boneweight는 3개까지만 저장. 나머지 하나는 1 - (weight0 + weight1 + weight2)로 계산
    // bone에 바인딩 된 mesh의 vertex 정보
    ...
    struct M3dMaterial
    {
        ...
        {
            size_t firstIdx = DiffuseMapName.rfind(L'/') + 1;
            size_t lastIdx = DiffuseMapName.rfind(L'.');
            return DiffuseMapName.substr(firstIdx, lastIdx - firstIdx);
        }
        
        std::string GetNormalMap()
        {
			size_t firstIdx = NormalMapName.rfind(L'/') + 1;
			size_t lastIdx = NormalMapName.rfind(L'.');
			return NormalMapName.substr(firstIdx, lastIdx - firstIdx);
        }
    };
